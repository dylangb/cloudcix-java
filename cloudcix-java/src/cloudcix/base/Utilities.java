...
public class Utilities {
	...
	public void get_new_admin_token() throws SocketTimeoutException {
		...
		if (response.code == 201) {
			...
			//TimerTask timerTask = new Scheduler(contextPath);
	...
	public void get_new_admin_token(CloudCIXAuth credentials) throws SocketTimeoutException {
		...
		if (response.code == 201) {
			setApi_token(response.headers.get("X-Subject-Token").get(0));
			//TimerTask timerTask = new Scheduler(contextPath, credentials);
			//Timer timer = new Timer(true);
			//timer.scheduleAtFixedRate(timerTask, 0, 55 * 60 * 1000);
		} else {
			try { 
				throw new Exception("Can't create administrative token, check your credentials please.");
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
